document.addEventListener('DOMContentLoaded', () => {
    ...
    showMoreButton.classList.add('p-2', 'bg-blue-500', 'text-white', 'rounded', 'shadow', 'mt-4');
    let currentPage = 1;
    const pageSize = 5;
    let currentUrl = '';
    let showImages = true;
    ...
    const fetchNews = (url, title, append = false) => {
      ...
      fetch(url)
        ...
        .then(data => {
          if (!append) {
            newsList.innerHTML = ''; // Clear previous news only if not appending
          ...
          data.articles.forEach(article => {
            ...
            listItem.classList.add('p-4', 'border', 'border-gray-300', 'rounded', 'bg-white', 'shadow-sm');
